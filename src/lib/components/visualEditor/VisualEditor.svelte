<script lang="ts">
  ...
  let ghost = $state<{ x: number; y: number; label: string } | undefined>(undefined);
  ...
  const onPointerMove = (event: PointerEvent) => {
    ...
    if (gesture.kind === 'node' && gesture.id) {
      ...
      ghost = { x: event.clientX, y: event.clientY, label: node?.label || gestureId };
      setDropTarget(clusterAt(event.clientX, event.clientY)?.el);
    }
  };
  ...
  <div class="relative flex-1 overflow-hidden">
    ...
    {#if !visualEditor.unsupported}
      ...
      {#if !visualEditor.selectedId}
        <div
          class="pointer-events-none absolute bottom-2 left-1/2 -translate-x-1/2 rounded-full bg-muted/90 px-3 py-1 text-xs text-muted-foreground shadow">
          Click a node to edit it · Drag a node onto a group to move it
        </div>
      {/if}
    {/if}
    ...
    {#if ghost}
      <div
        class="pointer-events-none fixed z-50 -translate-x-1/2 -translate-y-1/2 rounded-md border border-accent bg-background px-2 py-1 text-xs font-medium shadow-lg"
