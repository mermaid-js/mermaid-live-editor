<script lang="ts">
  ...
  import { getDomain } from '$lib/util/util';
  ...
  // Vendored locally to avoid external CDN requests
  const FONT_AWESOME_URL = `/vendor/font-awesome/${FAVersion}/css/all.min.css`;
...
<Card title="Actions" isStackable icon={{ component: DownloadIcon, class: 'rotate-180' }}>
  <div class="flex min-w-fit flex-col gap-2 p-2">
    ...
    {/if}
    <Separator />
    <p class="text-xs text-muted-foreground p-1">Diagram state is saved to the current URL. Diagrams are hence shareable as links. :-)</p>
  </div>
</Card>
